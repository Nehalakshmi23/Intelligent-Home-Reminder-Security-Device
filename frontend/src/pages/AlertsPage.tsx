...
const AlertsPage: React.FC = () => {
  ...
  const fetchAlerts = async () => {
    ...
    if (data.length > 0) {
      ...
      if (latestAlert.id !== lastAlertIdRef.current && latestAlert.type !== 'motion') {
        // Play sound for new alert (except motion which is low priority)
        playSound(latestAlert.type);
        lastAlertIdRef.current = latestAlert.id;
      } else if (latestAlert.id !== lastAlertIdRef.current) {
        lastAlertIdRef.current = latestAlert.id;
      }
    }
